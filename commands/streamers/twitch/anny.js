...
class ViwersCommand extends RateLimitedTwitchChatCommand {
  ...
  async delayRun(msg) {
    try {
      let info = await twitchSvc.getLiveViewersCountByName('anny');
      if ( info.viewers ){
        msg.reply(`anny台現在有:${info.viewers}人`);
      }else{
        if ( info.time ){
          msg.reply('anny現在沒開台，上次關台是'+ moment( info.time).format("yyyy-MM-DD HH:mm"));  
        }else{
          msg.reply('anny現在沒開台');
        } 
      }
